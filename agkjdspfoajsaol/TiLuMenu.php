...
<button style="border-radius:10px" name="name" type="submit" value="TiLu" style="border-radius:5">Map</button>You</h1></form>
<hr>
</div>
<?php
$hosp=$_POST["hosp"];
...
$fish=$_POST["fish"];
$ston=$_POST["ston"];
$L=file_get_contents("TiLuLevel");
if($hosp=="1"){
  $H=file_get_contents("TiLudeadsold.txt");
  $W=file_get_contents("TiLuWood");
  $S=file_get_contents("TiLuStone");
  $F=file_get_contents("TiLuFish");
  $L=file_get_contents("TiLuLevel");
  if($H-10<0||$W-10<0||$S-10<0||$F-10<0){
  }
  else{
    $h=@fopen("TiLudeadsold.txt","w");
    fwrite($h,$H-10);
    $w=@fopen("TiLuWood","w");
    fwrite($w,$W-10);
    $H=file_get_contents("TiLusold.txt");
    $h=@fopen("TiLusold.txt","w");
    fwrite($h,$H+10*$L);
    $w=@fopen("TiLuStone","w");
    fwrite($w,$S-10);
    $w=@fopen("TiLuFish","w");
...
if($forr==1){
  $H=file_get_contents("TiLuWood");
  $h=@fopen("TiLuWood","w");
  fwrite($h,$H+9*$L);
}
if($fish==1){
  $H=file_get_contents("TiLuFish");
  $h=@fopen("TiLuFish","w");
  fwrite($h,$H+9*$L);
}
if($ston==1){
  $H=file_get_contents("TiLuStone");
  $h=@fopen("TiLuStone","w");
  fwrite($h,$H+9*$L);
}
?>
