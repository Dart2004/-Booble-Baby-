...
<font face="Azonix">
<body style="background:black" text=white> <hr>
<hr>
<hr>
<div style="text-align:center">
<h1><form action="card.php" method='post'>
<button style="border-radius:10px" name="name" type="submit" value="JoshFoos" style="border-radius:5">Map</button>You</h1></form>
<hr>
</div>
<?php
$hosp=$_POST["hosp"];
...
$fish=$_POST["fish"];
$ston=$_POST["ston"];
$L=file_get_contents("JoshFoosLevel");
if($hosp=="1"){
  $H=file_get_contents("JoshFoosdeadsold.txt");
  $W=file_get_contents("JoshFoosWood");
  $S=file_get_contents("JoshFoosStone");
  $F=file_get_contents("JoshFoosFish");
  $L=file_get_contents("JoshFoosLevel");
  if($H-10<0||$W-10<0||$S-10<0||$F-10<0){
  }
  else{
    $h=@fopen("JoshFoosdeadsold.txt","w");
    fwrite($h,$H-10);
    $w=@fopen("JoshFoosWood","w");
    fwrite($w,$W-10);
    $H=file_get_contents("JoshFoossold.txt");
    $h=@fopen("JoshFoossold.txt","w");
    fwrite($h,$H+10*$L);
    $w=@fopen("JoshFoosStone","w");
    fwrite($w,$S-10);
    $w=@fopen("JoshFoosFish","w");
    fwrite($w,$F-10);
  }
}
if($barr==1){
...
if($forr==1){
  $H=file_get_contents("JoshFoosWood");
  $h=@fopen("JoshFoosWood","w");
  fwrite($h,$H+9*$L);
}
if($fish==1){
  $H=file_get_contents("JoshFoosFish");
  $h=@fopen("JoshFoosFish","w");
  fwrite($h,$H+9*$L);
}
if($ston==1){
  $H=file_get_contents("JoshFoosStone");
  $h=@fopen("JoshFoosStone","w");
  fwrite($h,$H+9*$L);
}
?>
<table>
