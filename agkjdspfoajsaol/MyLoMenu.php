...
<button style="border-radius:10px" name="name" type="submit" value="MyLo" style="border-radius:5">Map</button>You</h1></form>
<hr>
</div>
<?php
$hosp=$_POST["hosp"];
...
$fish=$_POST["fish"];
$ston=$_POST["ston"];
$L=file_get_contents("MyLoLevel");
if($hosp=="1"){
  $H=file_get_contents("MyLodeadsold.txt");
  $W=file_get_contents("MyLoWood");
  $S=file_get_contents("MyLoStone");
  $F=file_get_contents("MyLoFish");
  $L=file_get_contents("MyLoLevel");
  if($H-10<0||$W-10<0||$S-10<0||$F-10<0){
  }
  else{
    $h=@fopen("MyLodeadsold.txt","w");
    fwrite($h,$H-10);
    $w=@fopen("MyLoWood","w");
    fwrite($w,$W-10);
    $H=file_get_contents("MyLosold.txt");
    $h=@fopen("MyLosold.txt","w");
    fwrite($h,$H+10*$L);
    $w=@fopen("MyLoStone","w");
    fwrite($w,$S-10);
    $w=@fopen("MyLoFish","w");
...
if($forr==1){
  $H=file_get_contents("MyLoWood");
  $h=@fopen("MyLoWood","w");
  fwrite($h,$H+9*$L);
}
if($fish==1){
  $H=file_get_contents("MyLoFish");
  $h=@fopen("MyLoFish","w");
  fwrite($h,$H+9*$L);
}
if($ston==1){
  $H=file_get_contents("MyLoStone");
  $h=@fopen("MyLoStone","w");
  fwrite($h,$H+9*$L);
}
?>
