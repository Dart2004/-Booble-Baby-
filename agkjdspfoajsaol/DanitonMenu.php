<style>
  @font-face {font-family: Azonix; src: url("Azonix.otf");}
</style>
<font face="Azonix">
<body style="background:black" text=white> <hr>
<hr>
<hr>
<div style="text-align:center">
<h1><form action="card.php" method='post'>
<button style="border-radius:10px" name="name" type="submit" value="Daniton" style="border-radius:5">Map</button>You</h1></form>
<hr>
</div>
<?php
$hosp=$_POST["hosp"];
$barr=$_POST["barr"];
$forr=$_POST["forr"];
$fish=$_POST["fish"];
$ston=$_POST["ston"];
$L=file_get_contents("DanitonLevel");
if($hosp=="1"){
  $H=file_get_contents("Danitondeadsold.txt");
  $W=file_get_contents("DanitonWood");
  $S=file_get_contents("DanitonStone");
  $F=file_get_contents("DanitonFish");
  $L=file_get_contents("DanitonLevel");
  if($H-10<0||$W-10<0||$S-10<0||$F-10<0){
  }
  else{
    $h=@fopen("Danitondeadsold.txt","w");
    fwrite($h,$H-10);
    $w=@fopen("DanitonWood","w");
    fwrite($w,$W-10);
    $H=file_get_contents("Danitonsold.txt");
    $h=@fopen("Danitonsold.txt","w");
    fwrite($h,$H+10*$L);
    $w=@fopen("DanitonStone","w");
    fwrite($w,$S-10);
    $w=@fopen("DanitonFish","w");
    fwrite($w,$F-10);
  }
}
if($barr==1){
  $W=file_get_contents("DanitonWood");
  $S=file_get_contents("DanitonStone");
  $F=file_get_contents("DanitonFish");
  if($W-10<0||$S-10<0||$F-10<0){
  }
  else{
    $H=file_get_contents("Danitonsold.txt");
    $h=@fopen("Danitonsold.txt","w");
    fwrite($h,$H+1*$L);
    $w=@fopen("DanitonWood","w");
    fwrite($w,$W-10);
    $w=@fopen("DanitonStone","w");
    fwrite($w,$S-10);
    $w=@fopen("DanitonFish","w");
    fwrite($w,$F-10);
  }
}
if($forr==1){
  $H=file_get_contents("DanitonWood");
  $h=@fopen("DanitonWood","w");
  fwrite($h,$H+9*$L);
}
if($fish==1){
  $H=file_get_contents("DanitonFish");
  $h=@fopen("DanitonFish","w");
  fwrite($h,$H+9*$L);
}
if($ston==1){
  $H=file_get_contents("DanitonStone");
  $h=@fopen("DanitonStone","w");
  fwrite($h,$H+9*$L);
}
?>
<table>
<td style="width:calc(100% - 50px)">
<?php
  include "DanitonBuild";
?>
</td>
<td style="text-align:right;width:50px">
<?php
  $W=file_get_contents("DanitonWood");
  $S=file_get_contents("DanitonStone");
  $F=file_get_contents("DanitonFish");
  include "Wood";
  echo $W;
  echo "<br>";
  include "Ston";
  echo $S;
  echo "<br>";
  include "Fish";
  echo $F;
if($fish==2){
  $W=file_get_contents("DanitonWood");
  $S=file_get_contents("DanitonStone");
  $F=file_get_contents("DanitonFish");
  if($W-1000<0||$S-1000<0){
  }
  else{
  $w=@fopen("DanitonWood","w");
  fwrite($w,$W-1000);
  $s=@fopen("DanitonStone","w");
  fwrite($s,$S-1000);
  $f=@fopen("DanitonFish","w");
  fwrite($f,$F-1000);
  $level=file_get_contents("DanitonLevel");
  $lev=@fopen("DanitonLevel","w");
  fwrite($lev,$level+1/3);
  $build=file_get_contents("DanitonBuild");
  $building=@fopen("DanitonBuild","w");
  fwrite($building,$build.'<td><form action="DanitonMenu.php" method="post"><button style="border-radius:10px" name="fish" value=1 type="submit" style="background-color:white;border:none"><img style="width:40px;height:40px" src="data:image/png;base64,"></button></form></td>');
  }
}
if($forr==2){
  $W=file_get_contents("DanitonWood");
  $S=file_get_contents("DanitonStone");
  $F=file_get_contents("DanitonFish");
  if($F-1000<0||$S-1000<0){
  }
  else{
  $w=@fopen("DanitonWood","w");
  fwrite($w,$W-1000);
  $s=@fopen("DanitonStone","w");
  fwrite($s,$S-1000);
  $f=@fopen("DanitonFish","w");
  fwrite($f,$F-1000);
  $level=file_get_contents("DanitonLevel");
  $lev=@fopen("DanitonLevel");
  fwrite($lev,$level+1/3,"w");
  $build=file_get_contents("DanitonBuild");
  $building=@fopen("DanitonBuild","w");
  fwrite($building,$build.'<td><form action="DanitonMenu.php" method="post"><button style="border-radius:10px" name="forr" value=1 type="submit" style="background-color:white;border:none"><img style="width:40px;height:40px" src="data:image/png;base64,"></button></form></td>');
  }
}
if($ston==2){
  $W=file_get_contents("DanitonWood");
  $S=file_get_contents("DanitonStone");
  $F=file_get_contents("DanitonFish");
  if($W-1000<0||$F-1000<0){
  }
  else{
  $w=@fopen("DanitonWood","w");
  fwrite($w,$W-1000);
  $s=@fopen("DanitonStone","w");
  fwrite($s,$S-1000);
  $f=@fopen("DanitonFish","w");
  fwrite($f,$F-1000);
  $level=file_get_contents("DanitonLevel");
  $lev=@fopen("DanitonLevel","w");
  fwrite($lev,$level+1/3);
  $build=file_get_contents("DanitonBuild");
  $building=@fopen("DanitonBuild","w");
  fwrite($building,$build.'<td><form action="DanitonMenu.php" method="post"><button style="border-radius:10px" name="ston" value=1 type="submit" style="background-color:white;border:none"><img style="width:40px;height:40px" src="data:image/png;base64,"></button></form></td>');
  }
}
?>
</td>
</tr>
<tr style="text-align:center">
<h1>Building</h1>
</tr><tr>
<td><form action="" method="post">
<button style="border-radius:10px" name="ston" value="2">Add Mountain</button>
</form><td>
<td><form action="" method="post">
<button style="border-radius:10px" name="forr" value="2">Add Forrest</button>
</form><td>
<td><form action="" method="post">
<button style="border-radius:10px" name="fish" value="2">Add Bay</button>
</form><td>