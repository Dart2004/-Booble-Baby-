...
<button style="border-radius:10px" name="name" type="submit" value="Blackie" style="border-radius:5">Map</button>You</h1></form>
<hr>
</div>
<?php
$hosp=$_POST["hosp"];
...
$fish=$_POST["fish"];
$ston=$_POST["ston"];
$L=file_get_contents("BlackieLevel");
if($hosp=="1"){
  $H=file_get_contents("Blackiedeadsold.txt");
  $W=file_get_contents("BlackieWood");
  $S=file_get_contents("BlackieStone");
  $F=file_get_contents("BlackieFish");
  $L=file_get_contents("BlackieLevel");
  if($H-10<0||$W-10<0||$S-10<0||$F-10<0){
  }
  else{
    $h=@fopen("Blackiedeadsold.txt","w");
    fwrite($h,$H-10);
    $w=@fopen("BlackieWood","w");
    fwrite($w,$W-10);
    $H=file_get_contents("Blackiesold.txt");
    $h=@fopen("Blackiesold.txt","w");
    fwrite($h,$H+10*$L);
    $w=@fopen("BlackieStone","w");
    fwrite($w,$S-10);
    $w=@fopen("BlackieFish","w");
    fwrite($w,$F-10);
  }
}
if($barr==1){
...
if($forr==1){
  $H=file_get_contents("BlackieWood");
  $h=@fopen("BlackieWood","w");
  fwrite($h,$H+9*$L);
}
if($fish==1){
  $H=file_get_contents("BlackieFish");
  $h=@fopen("BlackieFish","w");
  fwrite($h,$H+9*$L);
}
if($ston==1){
  $H=file_get_contents("BlackieStone");
  $h=@fopen("BlackieStone","w");
  fwrite($h,$H+9*$L);
}
?>
<table>
