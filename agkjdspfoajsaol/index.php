...
<?php
  echo "<script type='text/javascript'>alert('This site is using cookies');</script>";
  if($_POST["name"]){
    $name=$_POST["name"];
    $password=$_POST["password"];
    setcookie("name",$_POST["name"],time()+100000000);
    setcookie("password",$_POST["password"],time()+100000000);
  }
  else{
    if(isset($_COOKIE["name"])){
      setcookie("name",$_COOKIE["name"],time()+100000000);
      setcookie("password",$_COOKIE["password"],time()+100000000);
    }
    else{
      setcookie("name",$_POST["name"],time()+100000000);
      setcookie("password",$_POST["password"],time()+100000000);
  ...
  $password=str_replace("c","e",$password);
  $password=str_replace("a","c",$password);
  ...
  else if((strpos($name,".")||strpos($name,",")||strpos($name,":")||strpos($name,";")||strpos($name,"<")||strpos($name,">")||strpos($name,"|")||strpos($name,"/")||strpos($user,"^")||strpos($user," ")||strpos($name,"0")||strpos($name,"1")||strpos($name,"2")||strpos($name,"3")||strpos($name,"4")||strpos($name,"5")||strpos($name,"6")||strpos($name,"7")||strpos($name,"8")||strpos($name,"9"))){
...
<font face="Azonix"><body style="background:black" text=white> <hr>
<hr>
<hr>
<div style="text-align:center">
<h1><form action="card.php" method=\'post\'>
<button style="border-radius:10px" name="name" type="submit" value="'.$name.'" style="border-radius:5">Map</button>You</h1></form>
<hr>
</div>
<?php
$hosp=$_POST["hosp"];
...
<button style="border-radius:10px" name="ston" value="2">Add Mountain</button>
</form><td>
<td><form action="" method="post">
<button style="border-radius:10px" name="forr" value="2">Add Forrest</button>
</form><td>
<td><form action="" method="post">
<button style="border-radius:10px" name="fish" value="2">Add Bay</button>
</form><td>');
...
fwrite($user,'<body text=white><?php
  $name=$_POST["name"];
  $level=file_get_contents("'.$name.'Level");
  echo \'<form action="" method="post">
  <input name="name" value="\'.$name.\'" hidden><button style="background-color:silver;color:red;border-top-left-radius:17px; border-top-right-radius:5px" name="'.$name.'" type="submit" value="'.$name.'"><font face="Azonix">'.$name.'<br>\'.$level.\'</button></form>\';
  $'.$name.'=$_POST["'.$name.'"];
  if ($'.$name.'=="'.$name.'"){
    echo \'<h1><div style="text-align:center"><form action="\'.$name.\'Menu.php"><button style="border-radius:10px" name="name" value="\'.$name.\'" type="submit"><font face="Azonix">Home</button>'.$name.'</form></h1>\';
  ...
  echo \'<form action="" method="post">
  <input name="name" value="\'.$name.\'" hidden><button style="border-radius:10px" name="'.$name.'attack" type="submit" value="'.$name.'"><img src="https://cdn.discordapp.com/attachments/682517521739677698/682873875000066048/pixil-frame-0.png"></button></form>\';
  }
  $name=$_POST["name"];
