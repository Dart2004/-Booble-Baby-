...
<button style="border-radius:10px" name="name" type="submit" value="Cody" style="border-radius:5">Map</button>You</h1></form>
<hr>
</div>
<?php
$hosp=$_POST["hosp"];
...
$fish=$_POST["fish"];
$ston=$_POST["ston"];
$L=file_get_contents("CodyLevel");
if($hosp=="1"){
  $H=file_get_contents("Codydeadsold.txt");
  $W=file_get_contents("CodyWood");
  $S=file_get_contents("CodyStone");
  $F=file_get_contents("CodyFish");
  $L=file_get_contents("CodyLevel");
  if($H-10<0||$W-10<0||$S-10<0||$F-10<0){
  }
  else{
    $h=@fopen("Codydeadsold.txt","w");
    fwrite($h,$H-10);
    $w=@fopen("CodyWood","w");
    fwrite($w,$W-10);
    $H=file_get_contents("Codysold.txt");
    $h=@fopen("Codysold.txt","w");
    fwrite($h,$H+10*$L);
    $w=@fopen("CodyStone","w");
    fwrite($w,$S-10);
    $w=@fopen("CodyFish","w");
...
if($forr==1){
  $H=file_get_contents("CodyWood");
  $h=@fopen("CodyWood","w");
  fwrite($h,$H+9*$L);
}
if($fish==1){
  $H=file_get_contents("CodyFish");
  $h=@fopen("CodyFish","w");
  fwrite($h,$H+9*$L);
}
if($ston==1){
  $H=file_get_contents("CodyStone");
  $h=@fopen("CodyStone","w");
  fwrite($h,$H+9*$L);
}
?>
