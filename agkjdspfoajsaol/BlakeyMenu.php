...
<button style="border-radius:10px" name="name" type="submit" value="Blakey" style="border-radius:5">Map</button>You</h1></form>
<hr>
</div>
<?php
$hosp=$_POST["hosp"];
...
$fish=$_POST["fish"];
$ston=$_POST["ston"];
$L=file_get_contents("BlakeyLevel");
if($hosp=="1"){
  $H=file_get_contents("Blakeydeadsold.txt");
  $W=file_get_contents("BlakeyWood");
  $S=file_get_contents("BlakeyStone");
  $F=file_get_contents("BlakeyFish");
  $L=file_get_contents("BlakeyLevel");
  if($H-10<0||$W-10<0||$S-10<0||$F-10<0){
  }
  else{
    $h=@fopen("Blakeydeadsold.txt","w");
    fwrite($h,$H-10);
    $w=@fopen("BlakeyWood","w");
    fwrite($w,$W-10);
    $H=file_get_contents("Blakeysold.txt");
    $h=@fopen("Blakeysold.txt","w");
    fwrite($h,$H+10*$L);
    $w=@fopen("BlakeyStone","w");
    fwrite($w,$S-10);
    $w=@fopen("BlakeyFish","w");
    fwrite($w,$F-10);
  }
}
if($barr==1){
...
if($forr==1){
  $H=file_get_contents("BlakeyWood");
  $h=@fopen("BlakeyWood","w");
  fwrite($h,$H+9*$L);
}
if($fish==1){
  $H=file_get_contents("BlakeyFish");
  $h=@fopen("BlakeyFish","w");
  fwrite($h,$H+9*$L);
}
if($ston==1){
  $H=file_get_contents("BlakeyStone");
  $h=@fopen("BlakeyStone","w");
  fwrite($h,$H+9*$L);
}
?>
<table>
